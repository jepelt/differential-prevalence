...
d_scatter <- left_join(res_null, res_orig_summary, by = "method") |> 
  mutate(nudge_x_vals = case_when(method == "DiPPER"        ~ -0.06,
                                  method == "MaAsLin2 (DA)" ~  0.10,
                                  method == "MaAsLin3-DP" ~  0.00,
                                  method == "Wald" ~  0.02,
                                  TRUE ~  0),
         nudge_y_vals = case_when(method == "DiPPER" ~ 0.15,
                                  method == "MaAsLin2 (DA)" ~  0.15,
                                  method == "MaAsLin3-DP" ~  0.07,
                                  method == "Wald" ~  0.05,
                                  TRUE ~  0),
         alpha_point = case_when(method == "DiPPER" ~ 1.0,
                                 TRUE ~ 0.7)) |> 
  arrange(method == "DiPPER")
...
(p_scatter <- ggplot(d_scatter, aes(x = fdr, y = md_s0, color = method)) +
  ...
  geom_hline(yintercept = 0, linetype = 'solid', color = 'gray40',
             linewidth = .2) +
  geom_vline(xintercept = sl, linetype = 'dashed', color = 'gray40') +
  ...
  geom_text_repel(aes(label = method),
                  color = "black",
                  # nudge_x = d_scatter$nudge_x_vals,
                  # nudge_y = d_scatter$nudge_y_vals,
                  fontface = 'plain',
                  size = 3.5, 
                  box.padding = 0.6,   
                  point.padding = 0.5,
  ...
  scale_x_continuous(breaks = seq(0, 0.25, 0.05), limits = c(0, 0.14)) +
  scale_y_continuous(breaks = seq(0, 30, 2), limits = c(0, 3.5)) +
...
ggsave(plot = p_final,
       filename = paste0('fig_s5_05_', date, '.png'),
...
writexl::write_xlsx(res_fig_2, paste0('res_fig_s5_05_', date, '.xlsx'))
