...
results_dipper <- res_dipper_default |>  
  map(~ .$res_beta) |> 
  bind_rows() |> 
...
res_orig <- res |> 
  ...
  filter(!is.na(q)) |>
  mutate(sgn = q < sl) |> 
  group_by(data_id, method, n_taxa) |> 
  summarize(s0 = sum(sgn),
            .groups = 'drop') |> 
  mutate(m0 = s0 / n_taxa) |> 
  group_by(data_id) |> 
  mutate(any_sgn = any(s0 > 0)) |> 
  ungroup() |> 
  filter(any_sgn)
...
(p_scatter <- ggplot(d_scatter, aes(x = fdr, y = md_m0, color = method)) +
    ...
    labs(x = 'Error rate\n(Proportion of null datsets with any significant findings)',
         y = 'Sensitivity\n(Median proportion of significant findings in original datasets)') +
...
md_q75 <- function(x) {
  median(x) + 0.00001 * quantile(x, 0.75)
}

d_distr <- res_orig |> 
  ungroup() |> 
  mutate(method = fct_reorder(method, m0, .fun = md_q75))


log10p <- function(x) log10(x + 0.01)

(p_distr <- ggplot(d_distr, aes(method, log10p(m0), color = method)) + 
    ggbeeswarm::geom_quasirandom(size = 1.2, alpha = .3, width = 0.3,
                                 bandwidth = 0.5) +
    geom_boxplot(alpha = .0, outlier.shape = NA, width = 0.6) +
    
    scale_y_continuous(breaks = log10p(c(0, .03, .10, 0.30)),
                       labels = c('0.00', '0.03', '0.10', '0.30')) +
    scale_color_manual(values = colors) +
    labs(y = 'Proportion of significant features in original datasets') +
    theme_classic(base_size = 11) +
...
(p_final <- free(p_scatter, type = "label") + free(p_distr, type = "label") +
    ...
    theme(plot.tag = element_text(face = "bold"),
          plot.margin = margin(t = 1, r = 1, b = -6, l = 1, unit = "mm"))
)

date <- format(Sys.Date(), "%d%m%y")
ggsave(plot = p_distr, filename = paste0('fig_s2_proportions_', date, '.png'),
       width = 90, height = 120, dpi = 300, unit = 'mm', 
       bg = 'white')
...
writexl::write_xlsx(res_fig_2, paste0('res_fig_s2_proportions_', date, '.xlsx'))