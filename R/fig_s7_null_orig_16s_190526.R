...
nrow(res |> filter(str_detect(data_id, 'original')) |> distinct(data_id))
...
(p_scatter <- ggplot(d_scatter, aes(x = fdr, y = md_s0, color = method)) +
    ...
    scale_x_continuous(breaks = seq(0, 0.25, 0.05), limits = c(0, 0.24)) +
...
ggsave(plot = p_final,
       filename = paste0('fig_s7_null_orig_16s_', date, '.png'),
...
writexl::write_xlsx(res_fig_2, paste0('res_fig_s7_null_orig_16s_', date, '.xlsx'))
