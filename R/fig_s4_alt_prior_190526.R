...
(p3 <- ggplot(res_sep_2, aes(x = c, y = m, color = method, shape = sl_lab)) +
    ...
    coord_cartesian(xlim = c(0, 125), ylim = c(0, 1150)) +
    scale_color_manual(values = colors,
...
ggsave(plot = p, filename = paste0('fig_s4_alt_prior_', date, '.png'),
...
writexl::write_xlsx(res_fig_3, paste0('res_fig_s4_alt_prior_', date, '.xlsx'))
