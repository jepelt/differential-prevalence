...
res_dipper <- res_dipper_default |> 
  map(~ .$res_beta) |> 
  bind_rows() |> 
  filter(data_id %in% c(null_data, original_data)) |> 
  mutate(method = 'DiPPER') |> 
  select(method, data_id, taxon, est = est, lwr = lwr90, upr = upr90)

res_freq |> group_by(data_id) |> summarise(n = n())
res_bonf |> group_by(data_id) |> summarise(n = n())
res_dipper |> group_by(data_id) |> summarise(n = n())

setdiff(res_dipper$taxon, res_bonf$taxon)


#-------------------------------------------------------------------------------
res_null <- bind_rows(res_freq, res_bonf, res_dipper) |> 
...
p1 <- ggplot(res_null, aes(x = est, y = taxon, color = Result)) +
  ...
  geom_point(size = 1) +
  geom_errorbarh(aes(xmin = lwr, xmax = upr), width = 0.0) +
  facet_grid(. ~ method) +
  scale_x_continuous(
  ...
  labs(x = 'Differential prevalence (OR)',
       title = 'Null dataset (no true differences)') + 
...
#-------------------------------------------------------------------------------
res_orig <- bind_rows(res_freq, res_bonf, res_dipper) |> 
  left_join(meta_taxa, by = c('data_id', 'taxon')) |>
  left_join(metas, by = 'data_id') |>
...
(p2 <- ggplot(res_orig_1, aes(x = est, y = taxon, color = Result)) +
   ...
   labs(x = 'Differential prevalence (OR)',
        title = 'Colorectal cancer vs. healthy') +
   scale_y_discrete(limits = rev) +
   coord_cartesian(xlim = c(-5.5, 5.5)) +
   scale_color_manual(
     values = c('Excludes OR = 1\n(significant)' = '#FA7413',
